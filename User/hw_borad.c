...
void TIM1_Init(void)
{
    ...
    TIM_TimeBaseInitTypeDef.TIM_Prescaler=48-1;   //预分屏
    TIM_TimeBaseInitTypeDef.TIM_CounterMode=TIM_CounterMode_Up;  // 向上计数
    TIM_TimeBaseInitTypeDef.TIM_Period=1000-1; //重装载值
    TIM_TimeBaseInitTypeDef.TIM_ClockDivision=TIM_CKD_DIV1; //不分屏
    TIM_TimeBaseStructInit(&TIM_TimeBaseInitTypeDef);

    TIM_UpdateRequestConfig(TIM1,TIM_UpdateSource_Regular);//请求更新中断源
    TIM_GenerateEvent(TIM1,TIM_EventSource_Update);   //人为产生更新事件
